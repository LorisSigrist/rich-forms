...
describe("Nested Form Data", () => {
    ...
    bench("parse (no arrays)", () => {
        const data = parse(formData, { populate_arrays: false , populate_booleans: true});
    });

    bench("parse (no booleans)", () => {
        const data = parse(formData, { populate_booleans: false, populate_arrays: true });
    });

    bench("parse (no arrays, no booleans)", () => {
        const data = parse(formData, { populate_arrays: false, populate_booleans: false });
    });
});